...
# merged_proxy_crops_hr_df.to_csv(r"C:\Users\armen\Desktop\COEQWAL\Scripts\landiq crop\HR_proxy_crops.csv")
#%%
...
avg_hr_prices = openag_crops.groupby(['Crop Name', 'HR_NAME'])['price_2020'].mean()
avg_hr_prices_dict = avg_hr_prices.to_dict()


openag_crops['neighbor_avg_price'] = openag_crops.apply(
    lambda row: avg_neighboring_prices_dict.get((row['Crop Name'], row['County'])), axis=1)

openag_crops['hr_avg_price'] = openag_crops.apply(
    lambda row: avg_hr_prices_dict.get((row['Crop Name'], row['HR_NAME'])), axis=1)
...
openag_crops.to_csv(r"C:\Users\armen\Desktop\COEQWAL\Datasets\Outputs\openag_crops.csv")
#%%
...
def fill_missing_values_with_neighbor_and_hr_avg(df, column_name):
    ...
    # Calculate average HR-level values per Crop_OpenAg and HR_NAME
