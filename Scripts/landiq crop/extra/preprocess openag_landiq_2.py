...
import numpy as np
import ast
## only keep 2020 econ data of proxy crops in each county
...
## only keep econ data found in landiq
...
usda_crops = usda_crops.merge(landiq_openag_crops, on=['Crop_OpenAg', 'County'], how='right')
usda_crops = usda_crops.rename(columns={'Crop Name': 'usda crop'})


usda_crops = usda_crops[['usda crop', 'County','HR_NAME_y', 'Neighboring Counties_y', 'Neighboring HR_y', 
                         'price_2020', 'Production_2020', 'Acres_2020',
                         'yield_2020', 'Crop_OpenAg', 'ACRES']]

usda_crops.rename(columns={
    'HR_NAME_y': 'HR_NAME',
...
usda_crops_updated = usda_crops_updated[['Crop_OpenAg', 'usda crop', 'ACRES', 'County','HR_NAME', 'Neighboring Counties','Neighboring HR', 
                                          'price_2020', 'Production_2020', 'Acres_2020','yield_2020']]
#%%
...
usda_crops_updated['economic_data_availability'] = np.where(usda_crops_updated['price_2020'].isna(), 'N', 'Y')
usda_crops_updated.to_csv('../../Datasets/econ_crop_data/processed_usda_crops_20.csv')
