...
with open("crop_yield_data.pkl", "rb") as pickle_file:
...
price_change = (pivot_yield[2022] - pivot_yield[2018]) / pivot_yield[2018] * 100
price_change = price_change.reset_index()
price_change.columns = ['County', 'Yield Increase']
...
merged_gdf.plot(column='Yield Increase', cmap='OrRd', linewidth=0.8, ax=ax, edgecolor='0.8', legend=True)
for idx, row in merged_gdf.iterrows():
    centroid = row['geometry'].centroid
    ax.text(centroid.x, centroid.y, row['County'], fontsize=8, ha='center', color='black')
ax.set_title('Yield Increase by County', fontsize=15)
counties_gdf.boundary.plot(ax=ax, linewidth=1, color='black')

plt.show()

#%%
pivot_yield = pivot_yield.apply(lambda col: col.map(lambda x: np.nan if x > 100 else x))
df_yearly_change = pivot_yield.diff(axis=1)
df_wrt_2018 = pivot_yield.sub(pivot_yield[2018], axis=0)
# df_yearly_change = df_yearly_change.apply(lambda col: col.map(lambda x: np.nan if x > 2000 else x))
...
pivot_yield.T.plot(ax=axs[0], marker='o', legend=False)
